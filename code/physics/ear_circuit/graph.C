...
void graph()
{
    ...
    g_3->SetTitle("Risposta in frequenza del circuito risonante a frequenza minore");
    ...
    g_3->Draw();
    c_3->Print("primo_circuito.pdf");

    TCanvas *c_9 = new TCanvas("c_9", "Risposta in frequenza secondo circuito");
    c_9->SetGrid();
    TGraph *g_9 = new TGraph("V_R_9.dat", "%lg %lg");
    g_9->SetTitle("Risposta in frequenza del circuito risonante a frequenza maggiore");
    ...
    g_9->Draw();
    c_9->Print("secondo_circuito.pdf");

    TCanvas *c_sum = new TCanvas("c_sum", "Somma delle risposte in frequenza");
    c_sum->SetGrid();
    TGraph *g_sum = new TGraph("V_R_tot.dat", "%lg %lg");
    g_sum->SetTitle("Risposta in frequenza");
    g_sum->GetXaxis()->SetTitle("Frequenza (Hz)");
    g_sum->GetYaxis()->SetTitle("Ampiezza (V)");

    g_sum->Draw();
    c_sum->Print("somma_maglie.pdf");
}