...
// add padding to avoid cache invalidation when using multiple threads
...
int main()
{
  ...
  for (size_t i = 0; i < numThreads; ++i)
  {
    threads.emplace_back([&data, i]()
                         { data.counting(i); }); // Pass the index and data reference
  }
  ...
  std::cout << "Counter time taken: " << timer.elapsed() << " milliseconds"
            << std::endl;
  ...
  for (size_t i = 0; i < numThreads; ++i)
  {
    threads.emplace_back([&good_data, i]()
                         { good_data.counting(i); }); // Pass the index and data reference
  }
  ...
  std::cout << "GoodCounter time taken: " << timer.elapsed() << " milliseconds"
            << std::endl;
