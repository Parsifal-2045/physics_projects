...
Image scale(Image const& src, int width, int height) {
  ...
  tbb::parallel_for(tbb::blocked_range2d<int, int>(0, width, 16, 0, height, 16), [&](const auto& range2d) {
    for (int y = range2d.rows().begin(); y != range2d.rows().end(); ++y) {
      ...
      for (int x = range2d.cols().begin(); x != range2d.cols().end(); ++x) {
  ...
  if (verbose) {
    ...
    cerr << fmt::format("scale:      {:6.2f}", ms) << " ms\n";
  }
...
void write_to(Image const& src, Image& dst, int x, int y) {
  ...
  if (verbose) {
    std::osyncstream cerr{std::cerr};
    cerr << fmt::format("write_to:   {:6.2f}", ms) << " ms\n";
  }
}
...
Image grayscale(Image const& src) {
  ...
  tbb::parallel_for(tbb::blocked_range2d<int, int>(0, dst.height_, 16, 0, dst.width_, 16), [&](const auto& range2d) {
  ...
  if (verbose) {
    std::osyncstream cerr{std::cerr};
    cerr << fmt::format("grayscale:  {:6.2f}", ms) << " ms\n";
  }
...
Image tint(Image const& src, int r, int g, int b) {
  ...
  if (verbose) {
    std::osyncstream cerr{std::cerr};
    cerr << fmt::format("tint:       {:6.2f}", ms) << " ms\n";
  }
...
int main(int argc, const char* argv[]) {
  ...
  std::mutex m;
  tbb::parallel_for(
      tbb::blocked_range<int>(0, files.size(), 1),
      [&](const auto& range) {
